...
class Office365(models.Model):
    ...
    @api.depends("ks_code", "ks_scope", "ks_auth_token", "ks_auth_refresh_token")
    def ks_get_office_user(self):
        if len(self) > 1:
            ks_office_user = self.env['ks.office.res.users'].search([('ks_res_user_id', '=', self.env.user.id)], limit=1)
        else:
            ks_office_user = self.env['ks.office.res.users'].search([('ks_res_user_id', '=', self.id)], limit=1)
        if ks_office_user:
            self.update({"ks_res_users": ks_office_user.id})

    ks_res_users = fields.Many2one('ks.office.res.users', compute="ks_get_office_user", store=True)
    ...
    def ks_is_job_completed(self, ks_job, ks_module):
        ...
        is_ks_process_running = self.env["ks.office.job"].search(
            [('ks_records', '>=', 0), ('ks_status', '=', 'in_process'),
             ('ks_module', '=', ks_module), ('create_uid', '=', self.env.user.id)])
    ...
    def get_authentication_code(self):
        ...
        if not ks_client_id or not ks_client_secret or not ks_redirect_url:
            self.ks_create_log("authentication", "Authentication", "", 0, datetime.today(), "authentication",
                               ...
                               "Client ID or Client Secret or Redirect URI is not entered!")
    ...
    def ks_generate_token(self):
        ...
        if not ks_auth_user or not self.ks_code:
            self.ks_create_log("authentication", "Authentication", "", 0, datetime.today(), "authentication",
        ...
        if "access_token" in json_data:
            ...
            self.ks_create_log("authentication", "Authentication", "", 0, datetime.today(), "authentication",
                               "authentication", "success", "Token generated successfully!")
        else:
            self.ks_create_log("authentication", "Authentication", "", 0, datetime.today(), "authentication",
    ...
    def refresh_token(self):
        api_endpoint = "https://login.microsoftonline.com/common/oauth2/v2.0/token"
        ks_auth_user = self.env["ks_office365.settings"].search([])
        ks_client_id = ks_auth_user.ks_client_id
        ks_client_secret = ks_auth_user.ks_client_secret
        ks_redirect_url = ks_auth_user.ks_redirect_url
        if not ks_auth_user:
        ...
        if "access_token" in json_data:
            ...
            self.sudo().ks_create_log("authentication", "Authentication", "", 0, datetime.today(), "authentication",
