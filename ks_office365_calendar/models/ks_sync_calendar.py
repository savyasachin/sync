...
class KsOfficeUserCalendar(models.Model):
    _inherit = "ks.office.res.users"
    ...
    ks_sync_using_subject = fields.Boolean("Subject", default=True, readonly=True)
    ks_sync_using_start_datetime = fields.Boolean("Start DateTime", default=True)
    ks_sync_using_end_datetime = fields.Boolean("End DateTime", default=True)

    ks_calendar_sync_days_before = fields.Integer(string="Sync events from last", default=1,
            ...
            if 'Internal User' in user.groups_id.mapped('name'):
                if not user.ks_auth_token:
                    user.ks_create_log("authentication", user.name, "", 0, datetime.now(), "office_to_odoo",
                                       ...
                                       "Generate Authentication Token. '%s\' has not generated token yet." % user.name)
                    continue
                user.ks_post_events(ks_individual_event=None)

    def ks_get_events(self):
        res_user = self.env.user
        return res_user.sudo().ks_get_events()

    def ks_post_events(self, ks_individual_event=None):
        res_user = self.env.user
        return res_user.sudo().ks_post_events(ks_individual_event)
...
class KsOffice365Calendar(models.Model):
    ...
    ks_import_office_calendar = fields.Boolean("Import", default=True, readonly=True)
    ks_export_office_calendar = fields.Boolean("Export", default=True, readonly=True)
    ks_sync_using_subject = fields.Boolean(related='ks_res_users.ks_sync_using_subject', store=True)
    ks_sync_using_start_datetime = fields.Boolean(related='ks_res_users.ks_sync_using_start_datetime', store=True)
    ks_sync_using_end_datetime = fields.Boolean(related='ks_res_users.ks_sync_using_end_datetime', store=True)
    ks_calendar_sync_days_before = fields.Integer(related='ks_res_users.ks_calendar_sync_days_before', store=True)
    ks_calendar_sync_using_days = fields.Boolean(related='ks_res_users.ks_calendar_sync_using_days', store=True)
    ks_calendar_filter_domain = fields.Char(related='ks_res_users.ks_calendar_filter_domain', store=True)
    ks_sync_deleted_event = fields.Boolean(related='ks_res_users.ks_sync_deleted_event', store=True)
    ...
    def ks_import_events(self):
        ks_current_job = self.ks_is_job_completed("calendar_import", ks_module="calendar")
        ...
        if not ks_auth_token:
            self.ks_create_log("authentication", self.name, "", 0, ks_current_datatime, "office_to_odoo",
                               "authentication", "failed",
                               "Generate Authentication Token.\nCurrent user has not generated token yet.")
        ...
        ks_syncing_field = self.ks_calendar_syncing_fields()
        ...
        if 'subject' in ks_syncing_field and 'start' not in ks_syncing_field and 'end' not in ks_syncing_field:
            ...
            for ks_event in ks_all_office_events:
                if _subject.count(ks_event['subject']) == 1:
                    self.ks_create_log("calendar", ks_event['subject'], ks_event['id'], 0,
                                       ...
                                       "Multiple event with the same subject \'" + ks_event['subject'] +
            ...
            _subject_end = list()
            for ks_event in ks_all_office_events:
                _end = datetime.strptime(ks_event['start']['dateTime'].split('.')[0].replace('T', ' '),
                                         '%Y-%m-%d %H:%M:%S')
                if ks_event['recurrence']:
                    _end = ks_event['recurrence']['range']['endDate']
                if _subject_end.count((ks_event['subject'], _end)) == 1:
                    self.ks_create_log("calendar", ks_event['subject'], ks_event['id'], 0,
                                       ks_current_datatime, "office_to_odoo", False, "failed",
                                       "Multiple event with the same subject \'" + ks_event['subject'] +
                    ...
                    ks_office_duplicate_subject_end.add((ks_event['subject'], _end))
            ...
            for ks_event in ks_all_office_events:
                ...
                if _subject_start_end.count((ks_event['subject'], ks_event['start']['dateTime'],
                    ...
                    self.ks_create_log("calendar", ks_event['subject'], ks_event['id'], 0,
                                       ks_current_datatime, "office_to_odoo", False, "failed",
                                       "Multiple event with the same subject \'" + ks_event['subject'] +
                                       "\', start datetime \'" + str(_start) + "\' and end datetime \'" + str( _end) +
                                       "\' exists in in Office 365.\nNote: This event is being synced with respect to "
                                       "subject, start datetime and end datetime.")
                    ks_office_duplicate_subject_start_end.add((ks_event['subject'], _start, _end))
                _subject_start_end.append((ks_event['subject'], ks_event['start']['dateTime'], _end))
        ...
        for ks_e in ks_office_events:
            ...
            ks_responsible = self.ks_get_responsible_user(ks_e)
            ks_description = self.ks_get_meeting_description(ks_e.get('body'))
            ...
            ks_vals = {
                ...
                "stop": ks_end,
                "ks_user_id": self.id,
                "user_id": ks_responsible.id if ks_responsible else self.id,
                "ks_office_event_id": ks_e['id'],
                "allday": ks_e['isAllDay'],
                "show_as": ks_e['showAs'] if ks_e['showAs'] in ['free', 'busy'] else "busy",
                "start_datetime": ks_start_datetime,
                "stop_datetime": ks_stop_datetime,
                "start_date": ks_start_date,
                "stop_date": ks_stop_date,
                "alarm_ids": [(4, ks_alarm_ids.id)] if ks_alarm_ids else None,
            ...
            auth_setting = self.env['ks_office365.settings'].search([], limit=1)
            if ks_e['id'] in ks_existing_odoo_events.mapped('ks_office_event_id'):
                if 'subject' in ks_syncing_field and 'start' not in ks_syncing_field and 'end' not in ks_syncing_field:
                    ...
                    _end = datetime.strptime(ks_e['end']['dateTime'].split('.')[0].replace('T', ' '),
                                             '%Y-%m-%d %H:%M:%S')
                    if (ks_e['subject'], _end) in ks_office_duplicate_subject_end:
                        continue
                ...
                ks_event = self.ks_eliminate_duplicate_event(ks_event)
                auth_setting.ks_dont_sync = True
                ks_some_error = self.ks_update_events(ks_event, ks_e, ks_recurrency, ks_location,
                                                      ...
                                                      _fr, _sa, _su, ks_vals)
                auth_setting.ks_dont_sync = False
                ...
                if len(ks_event) > 1:
                    self.ks_create_log("calendar", ks_e['subject'], '', 0, ks_current_datatime,
                                       "office_to_odoo", "update", "failed",
                                       "Multiple event with the same subject \'" + ks_e['subject'] +
                                       "\' exists in Odoo.\nNote: This event is being synced with respect to "
                                       "subject.")
                    continue
                elif len(ks_event) == 1:
                    ...
                    event = self.ks_create_odoo_event(ks_vals, ks_e)
                ...
                if ks_e['recurrence']:
                    ...
                    ks_event = self.env['calendar.event'].search([('name', '=', ks_e['subject']), ('stop', '=', _end)])
                if len(ks_event) > 1:
                    self.ks_create_log("calendar", ks_e['subject'], '', 0, ks_current_datatime,
                                       "office_to_odoo", "update", "failed",
                                       "Multiple event with the same subject \'" + ks_e['subject'] +
                ...
                if ks_e['recurrence']:
                    ...
                    for _id in _ev_ids:
                        _ev = self.env['calendar.event'].browse(_id)
                        if _ev.name == ks_e['subject'] and str(_ev.start.date()) == _start and str(
                                _ev.final_date) == _final:
                            ks_event.append(_ev)
                else:
                    ks_event = self.env['calendar.event'].search([('name', '=', ks_e['subject']),
                                                                  ('start', '=', str(_start)),
                ...
                if len(ks_event) > 1:
                    self.ks_create_log("calendar", ks_e['subject'], '', 0, ks_current_datatime,
                                       "office_to_odoo", "update", "failed",
                                       "Multiple event with the same subject \'"
                                       + ks_e['subject'] + "\', start datetime \'" + ks_e['start']['dateTime'] +
                                       "\' and end datetime \'" + ks_e['end']['dateTime'] +
                                       "\' exists in Odoo.\nNote: This event is being synced with respect to "
                                       "subject, start datetime and end datetime.")
                    continue
                elif len(ks_event) == 1:
    ...
    def ks_get_meeting_description(self, meeting_body):
        if meeting_body and meeting_body.get('content'):
            description_soup = BeautifulSoup(meeting_body.get('content'), "lxml")
            return description_soup.text.strip('\n')
        return ""

    def ks_eliminate_duplicate_event(self, ks_events):
        final_event = ks_events[0]
        if len(ks_events) > 1:
            for event in ks_events[1:]:
                event.sudo().unlink()

        return final_event

    def ks_get_responsible_user(self, event):
        if event.get('organizer'):
            organiser_email = event.get('organizer').get('emailAddress').get('address')
            res_user = self.sudo().env['res.users'].search(['|', ('login', '=', organiser_email),
                                                            ('email', '=', organiser_email)])
            if not res_user:
                res_partner = self.env['res.partner'].search([('email', '=', organiser_email)])
                if res_partner:
                    return res_partner.user_ids[0] if res_partner.user_ids else False
                else:
                    return False
            else:
                return res_user
    ...
    def ks_sync_event_deleted_in_odoo(self, ks_office_event, header):
        odoo_meet = self.env['calendar.event'].search([('ks_office_event_id', '=', ks_office_event['id']),
                                                       ('ks_user_id', '=', self.id)])
        if odoo_meet:
            return False
    ...
    def ks_create_odoo_event(self, ks_vals, ks_e):
        office_settings = self.env['ks_office365.settings'].search([], limit=1)
        try:
            if office_settings.ks_create_meeting_invite and office_settings.ks_new_meeting_invite:
                event = self.env['calendar.event'].create(ks_vals)
                # while importing through cron unable to set privacy of other users if the meeting is private
                event.sudo().with_context(no_mail_to_attendees=True).write({
                    "privacy": 'public' if ks_e['sensitivity'] == 'normal' else 'private',
                })
                return event
            else:
                event = self.env['calendar.event'].with_context(no_mail_to_attendees=True).create(ks_vals)
                # while importing through cron unable to set privacy of other users if the meeting is private
                event.sudo().with_context(no_mail_to_attendees=True).write({
                    "privacy": 'public' if ks_e['sensitivity'] == 'normal' else 'private',
                })
            ...
            self.ks_create_log("calendar", ks_vals['name'], ks_vals['ks_office_event_id'], 0, datetime.today(),
                               "office_to_odoo", "create", "failed", "Failed at invitation context: " + str(ex))

    def ks_update_odoo_event_with_invitation(self, event, vals, ks_e):
        office_settings = self.env['ks_office365.settings'].search([], limit=1)
        try:
            vals.update({
                "privacy": 'public' if ks_e['sensitivity'] == 'normal' else 'private',
            })
            if office_settings.ks_create_meeting_invite and office_settings.ks_update_meeting_invite:
                # while importing through cron unable to set privacy of other users if the meeting is private
                event.sudo().write(vals)
            else:
                # while importing through cron unable to set privacy of other users if the meeting is private
                event.sudo().with_context(no_mail_to_attendees=True).write(vals)
        except Exception as ex:
            self.ks_create_log("calendar", event.name, event.ks_office_event_id, event.id, datetime.today(),
                               "office_to_odoo", "update", "failed", "Failed at invitation context: " + str(ex))
    ...
    def ks_update_events(self, event, ks_e, ks_recurrency, ks_location, ks_alarm_ids, ks_attendees, ks_pattern,
        ...
        try:
            if event[0].recurrency and ks_recurrency:
                ...
                ks_responsible = self.ks_get_responsible_user(ks_e)
                ks_recurr_vals = {
                    "description": ks_vals.get("description"),
                    "name": ks_e['subject'],
                    "location": ks_location,
                    "ks_user_id": self.id,
                    "user_id": ks_responsible.id if ks_responsible else self.id,
                ...
                self.ks_update_odoo_event_with_invitation(real_event, ks_recurr_vals, ks_e)
                real_event.with_context(no_mail_to_attendees=True).write({'rrule': real_event._rrule_serialize()})

                self.ks_create_log("calendar", real_event.name, real_event.ks_office_event_id, str(real_event.id),
                                   datetime.today(), "office_to_odoo", "update", "success", "Record updated!")
            elif event[0].recurrency:
                ks_real_id = event[0].id
                real_event = self.env['calendar.event'].browse(ks_real_id)
                self.ks_update_odoo_event_with_invitation(real_event, ks_vals, ks_e)
                self.ks_create_log("calendar", real_event.name, real_event.ks_office_event_id, str(real_event.id),
                                   datetime.today(), "office_to_odoo", "update", "success", "Record updated!")
            elif not event[0].recurrency and ks_recurrency:
                ks_vals.update({'rrule': event[0]._rrule_serialize()})
                self.ks_update_odoo_event_with_invitation(event, ks_vals, ks_e)
                self.ks_create_log("calendar", event.name, event.ks_office_event_id, str(event.id), datetime.today(),
                                   "office_to_odoo", "update", "success", "Record updated!")
            else:
                self.ks_update_odoo_event_with_invitation(event, ks_vals, ks_e)
                self.ks_create_log("calendar", event.name, event.ks_office_event_id, str(event.id), datetime.today(),
                                   "office_to_odoo", "update", "success", "Record updated!")
            ...
            self.ks_create_log("calendar", event.name, ks_e['id'], event.id,
                               datetime.today(), "office_to_odoo", "update", "failed", ex)
            ks_some_error = True
    ...
    def ks_calendar_syncing_fields(self):
        ks_syncing_field = list()
        if self.ks_sync_using_subject:
            ks_syncing_field.append("subject")
        if self.ks_sync_using_start_datetime:
            ks_syncing_field.append("start")
        if self.ks_sync_using_end_datetime:
            ks_syncing_field.append("end")
        return ks_syncing_field

    def ks_post_events(self, ks_individual_event):
        try:
    ...
    def ks_get_odoo_events(self, sync_from, all_events=False):
        ...
        if not all_events:
            ...
            ks_search_domain.append(('write_date', '>=', sync_from))
    ...
    def ks_export_events(self, ks_individual_event):
        ...
        if not ks_auth_token:
            self.ks_create_log("authentication", self.name, "", 0, ks_current_datatime,
                               "odoo_to_office", "authentication", "failed",
                               "Generate Authentication Token.\nCurrent "
                               "user has not generated token yet.")
            return self.ks_show_error_message(_('Generate Authentication Token.'))
        ...
        if not ks_individual_event:
            ks_current_job = self.ks_is_job_completed(ks_job="calendar_export", ks_module="calendar")
            if not ks_current_job:
                return self.ks_show_error_message(_('Process Is Already Running.'))
            else:
            ...
            if 'error' in ks_office_events_json_data:
                if ks_office_events_json_data["error"]['code'] == 'InvalidAuthenticationToken':
                    ...
                    if 'error' in ks_office_events_json_data:
                        self.ks_create_log("authentication", "Authentication", "", 0, ks_current_datatime,
                                           "odoo_to_office", "authentication", "failed",
                                           ks_office_events_json_data["error"]['code'])
                        ks_current_job.write({'ks_status': 'completed'})
                        return self.ks_show_error_message(
                            _("Some error occurred! \nPlease check logs for more information."))
                else:
                    self.ks_create_log("authentication", "Authentication", "", 0, ks_current_datatime,
                                       "odoo_to_office",
                                       "authentication", "failed", ks_office_events_json_data["error"]['code'])
                    ks_current_job.write({'ks_status': 'completed'})
                    return self.ks_show_error_message(
                        _("Some error occurred! \nPlease check logs for more information."))
            ...
            if 'subject' in ks_syncing_field and 'start' not in ks_syncing_field and 'end' not in ks_syncing_field:
                ...
                for ks_event in ks_all_odoo_events:
                    if _subject.count(ks_event.name) == 1:
                        self.ks_create_log("calendar", ks_event.name, ks_event.ks_office_event_id, ks_event.id,
                                           ...
                                           "Multiple events with same subject \'" + ks_event.name +
                ...
                for ks_event in ks_all_odoo_events:
                    if _subject_start_end.count((ks_event.name, ks_event.start, ks_event.stop)) == 1:
                        self.ks_create_log("calendar", ks_event.name, ks_event.ks_office_event_id, ks_event.id,
                                           ks_current_datatime, "odoo_to_office", False, "failed",
                                           "Multiple events with same subject \'" + ks_event.name + "\', start datetime \'"
                                           + str(ks_event.start) + "\' and end datetime \'" + str(
                                               ks_event.stop) +
                                           "\' exists in Odoo. \nNote: This event is being synced "
        ...
        for event in ks_odoo_events[ks_events_exported:]:
            ...
            if ks_real_id in ks_synced_events:
                continue
            else:
                event = self.env['calendar.event'].browse(ks_real_id)
                ks_synced_events.append(ks_real_id)
            ...
            if event.allday:
                _start = event.start.replace(hour=8)
                _stop = event.stop.replace(hour=18, minute=0, second=0)
                start_date = self.ks_datetime_from_utc_to_local(_start).split('T')[0] + "T00:00:00.0000000"
                if event.allday and event.stop.date() != event.start.date():
                ...
                start_date = self.ks_datetime_from_utc_to_local(event.start)
                stop_date = self.ks_datetime_from_utc_to_local(event.stop + relativedelta(seconds=10))
            ...
            data = {
                ...
                "showAs": event.show_as or 'tentative',
            ...
            if event.ks_office_event_id != "":
                ...
                if len(_office_id) > 1:
                    self.ks_create_log("event", event.name, event.ks_office_event_id, event.id,
                                       ...
                                       "Multiple events with same subject \'" + event.name +
                ...
                for ks_o_e in ks_office_events:
                    if event.allday:
                        ...
                        _start_odoo = event.start.date()
                    else:
                        _start_office = self.ks_str_datetime_to_datetime(ks_o_e['start']['dateTime'], 'start', allday=0)
                        _start_odoo = event.start
                    if ks_o_e['subject'] == event.name and _start_office == _start_odoo:
                        _office_id.append(ks_o_e['id'])

                if len(_office_id) > 1:
                    self.ks_create_log("event", event.name, event.ks_office_event_id, event.id,
                                       ks_current_datatime, "odoo_to_office", "update", "failed",
                                       "Multiple events with same subject \'" + event.name + "\', start datetime \'"
                                       + str(event.start) +
                                       "\' exists in Office 365. \nNote: This event is being synced "
                                       "with respect to subject and start datetime.")
                    continue
                ...
                for ks_o_e in ks_office_events:
                    if event.allday:
                        _end_office = self.ks_str_datetime_to_datetime(ks_o_e['end']['dateTime'], 'end', allday=1)
                        _end_odoo = event.stop.date()
                    else:
                        _end_office = self.ks_str_datetime_to_datetime(ks_o_e['end']['dateTime'], 'end', allday=0)
                        _end_odoo = event.stop

                    if event.recurrency:
                        _end_office = datetime.strptime(ks_o_e['recurrence']['range']['endDate'], '%Y-%m-%d').date()
                        _end_odoo = event.final_date

                    if ks_o_e['subject'] == event.name and _end_office == _end_odoo:
                        _office_id.append(ks_o_e['id'])

                if len(_office_id) > 1:
                    self.ks_create_log("event", event.name, event.ks_office_event_id, event.id,
                                       ks_current_datatime, "odoo_to_office", "update", "failed",
                                       "Multiple events with same subject \'" + event.name + "\' and end datetime \'"
                                       + str(event.stop) +
                                       "\' exists in Office 365. \nNote: This event is being synced "
                                       "with respect to subject and end datetime.")
                    continue
                ...
                for ks_o_e in ks_office_events:
                    ...
                    if ks_o_e['subject'] == event.name and _start_office == _start_odoo and _end_office == _end_odoo:
                        _office_id.append(ks_o_e['id'])
    ...
    def ks_sync_deleted_office_event(self, event, ks_head):
        ...
        if 'error' in ks_json_data:
            if ks_response.status_code == 404:
                if self.ks_sync_deleted_event:
                    del_event = {"name": event.name, "ks_office_event_id": event.ks_office_event_id, "id": event.id}
                    try:
                        event.sudo().unlink()
                        ...
                        self.ks_create_log("calendar", del_event['name'], del_event["ks_office_event_id"],
        ...
        if 'error' in ks_json_data:
            if ks_response.status_code == 404:
                ...
                if ks_json_data["error"]['code'] == 'InvalidAuthenticationToken':
                    ...
                    self.ks_create_log("calendar", event.name, event.ks_office_event_id, event.id.__str__(),
                                       datetime.today(), "odoo_to_office", "update", "failed", ks_error)
                    ks_some_error = True
            else:
                self.ks_create_log("calendar", event.name, event.ks_office_event_id, event.id.__str__(),
            ...
            if event.privacy == "private":
                ...
                event.write({"ks_office_event_id": ks_json_data['id'], 'ks_is_updated': False})
            self.ks_create_log("calendar", event.name, event.ks_office_event_id, event.id.__str__(),
    ...
    def ks_create_office_event(self, event, ks_head, data):
        ...
        if 'error' in ks_json_data:
            if 'code' in ks_json_data['error']:
                if ks_response.status_code == 429:
                    # when message limit is exceded, syncing is done but mail is not send to the attendees.
                    event.write({'ks_is_updated': False})
                    self.ks_create_log("calendar", event.name, event.ks_office_event_id, event.id.__str__(),
                                       datetime.today(), "odoo_to_office", "create", "failed",
                                       ks_json_data['error']['message'])
                elif ks_json_data["error"]['code'] == 'InvalidAuthenticationToken':
                    self.sudo().refresh_token()
                    ks_head['Authorization'] = self.ks_auth_token
            ...
            if event.privacy == "private":
                ...
                event.write({"ks_office_event_id": ks_json_data['id'], 'ks_is_updated': False})
            self.ks_create_log("calendar", event.name, event.ks_office_event_id, event.id.__str__(),
                               datetime.today(), "odoo_to_office", "create", "success", "Record created!")
        return ks_some_error
    ...
    def ks_get_event_recurrency(self, event):
        ...
        if event.end_type == "count":
            if event.rrule_type == "daily":
                occur = event.count
                interval = event.interval
                _ks_end_date = (event.stop + relativedelta(days=occur * interval - 1)).date()
            elif event.rrule_type == "weekly":
                _final_date = self.env['calendar.event'].search([('id', '=', event.id)], limit=1).stop
                ...
                _ks_end_date = event.start.date() + relativedelta(years=(event.interval * (event.count - 1)))
        else:
        ...
        ks_recurrence = {
            ...
            "range": {
                ...
                "startDate": str(event.start).split(' ')[0],
